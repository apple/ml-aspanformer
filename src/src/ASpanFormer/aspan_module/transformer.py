...
class messageLayer_gla(nn.Module):
    ...
    def forward(self, x0, x1, flow_feature0,flow_feature1,pos0,pos1,mask0=None,mask1=None,ds0=[4,4],ds1=[4,4]):
        ...
        """
        flow0,flow1=self.decode_flow(flow_feature0),self.decode_flow(flow_feature1)
    ...
    def decode_flow(self,flow_feature):
        ...
        flow_coordinates=torch.sigmoid(flow[:,:,:,:2])*torch.tensor([w,h]).cuda()[None,None,None]
        flow_var=flow[:,:,:,2:]
